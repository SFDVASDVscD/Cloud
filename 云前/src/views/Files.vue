...
<div class="exhibition1">
  ...
  <div class="name" >
    <span>我的文件</span>
    <div class="button">
      <el-button type="primary" round >创建文件夹</el-button>
    <div>
      ...
      <el-main>
        <div CLASS="upload-demo">
          <el-upload
            ...
            multiple
          >
            <el-button type="primary" round class="buttoni">上传文件</el-button>
          </el-upload>
        </div>
      </el-main>
    </el-container>
...
export default {
  ...
  methods: {
    //获取文件信息
    async init () {
        axios.get("http://localhost/file?type=").then((res) => {
        if (res.data.code == '1') {
    ...
    // 上传文件之前
    beforeUpload(file) {
      const isLt2M = file.size / 1024 / 1024 < 5000; // 文件大小是否小于100M
    
      if (!isLt2M) {
        this.$message.error("上传文件大小不能超过100MB！！！");
      }
      return  isLt2M; // 若为false则停止上传
    },
    // 上传文件
    uploadFile(param) {
      ...
      axios.post("http://localhost/file/upload", formDataInfo).then((res) => {
        if (res.data.code == "1") {
